...
| **消解闭环+健康气象** | 干预回执（已缓解/未缓解/已就医）驱动**涟漪消解率**量化（强度加权），干预有效性第一次可度量；**健康气象日报**用天气隐喻向患者/家属转译当日守护态势 | 华为/慢病管理App的干预=系统通知或人工问卷，无计算化触达时机、无回执加权消解、无证据存证 |
...
├── apps/patient-web            #   患者门户（新增「我的健康涟漪」：健康气象/涟漪曲线/守护回执/适老化模式）
...
# 2. 打开涟漪守护指挥中心大屏（医生工作台：五环涟漪图谱 + RII仪表 + 哈希链校验/FHIR导出）
#    http://localhost:5176
# 3. 打开患者端「健康涟漪」页（健康气象 + 涟漪曲线 + 守护回执 + 适老化模式）
#    http://localhost:5173/ripple

# 4. 运行端到端测试（22步：注册→分诊→挂号→病历→处方→随访→涟漪→RII→MDT→证据→回执消解→健康气象）
py -3 scripts/e2e_test.py

# 5. RippleBench v3 量化评测（60病例四域：分诊/涟漪/护栏/消解闭环）
...
- **E2E 22/22 通过**：完整患者旅程，含 RII / Timing Card / 反事实护栏 / FHIR Provenance / **干预回执消解** / **健康气象** 验证
- **RippleBench v3 四域全达标**：分诊top-1 **20/20** · 涟漪用例 **20/20** · 护栏**灵敏度100%/特异度100%** · **消解闭环10/10**
- **后端单测 11/11 通过**：五维图谱 / RII量化模型 / 反事实持久化 / 哈希链防篡改 / 时间学调度 / **干预回执与消解率** / **健康气象**
