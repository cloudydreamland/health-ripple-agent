...
## 第七/八轮：从"可信"到"可治理、可落地"

- **人机共驾终审**：智能体提出的每条守护计划，医生在指挥大屏上一键**通过 / 改期 / 否决**——否决立即从预报、天气、队列消失，审定理由随**印鉴链存证**（GUARD_PLAN_REVIEW 决策类型，患者越权审定 403 有 e2e 回归）。"终审权在医生"从此不是口号，是每一个按钮。
...
- **对话式回执**：患者端用自己的话说情况（"好多了 / 还是没好转 / 已经去医院"），本地确定性关键词解析为三态回执，解析不了就明说——不联网、不臆造；适老模式同步放大。
...
- 端到端测试 26/26 全过（注册 → 分诊 → 挂号 → 病历 → 处方 → 随访 → 涟漪推演 → 回执消解 → 健康气象 → 通知双通道 → **越权/未鉴权/患者越权审定安全负例** → **医生审定守护计划闭环（否决剔预报+沙盘+印鉴链）**）
