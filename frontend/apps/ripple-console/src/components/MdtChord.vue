...
const views = computed(() => {
  ...
  return Object.entries(props.mdt.consultation).map(([key, value]) => ({
    key,
    meta: AGENT_META[key] ?? { label: key, en: key, char: "?", color: "var(--muted)" },
    value,
  }));
});
    ...
    <div class="chord-side">
      <svg viewBox="0 0 336 340" class="chord-svg">
        ...
        <g v-for="a in agentDots" :key="'d' + a.key">
          <circle :cx="a.x" :cy="a.y" r="12" fill="var(--card)" :stroke="a.meta.color" stroke-width="2" />
          <text :x="a.x" :y="a.y + 4" text-anchor="middle" class="agent-char" :fill="a.meta.color">{{ a.meta.char }}</text>
          <text :x="a.lx" :y="a.ly" :text-anchor="a.anchor" class="agent-label" :fill="mdt ? a.meta.color : 'var(--muted)'">{{ a.meta.label.replace(" Agent", "") }}</text>
        </g>
        <!-- 中心 -->
...
.chord { fill: none; stroke: var(--line); stroke-width: 1.1; }
.chord.active { stroke: var(--line-strong); }
.agent-char { font-size: 11px; font-weight: 800; font-family: var(--font-sans); }
.agent-label { font-size: 10.5px; font-weight: 600; }
.center-num { font-size: 30px; font-weight: 700; font-family: var(--font-mono); fill: var(--ink); }
.center-cap { font-size: 9px; letter-spacing: 2px; fill: var(--muted); font-family: var(--font-mono); }
.center-sub { font-size: 8.5px; letter-spacing: 1.4px; fill: var(--faint); font-family: var(--font-mono); }
.chord-stats { display: grid; grid-template-columns: 1fr 1fr; gap: 4px 18px; width: 100%; max-width: 300px; font-size: 10px; letter-spacing: 1px; color: var(--muted); }
.chord-stats div { display: flex; justify-content: space-between; border-bottom: 1px dashed var(--line); padding: 3px 0; }
.chord-stats b { color: var(--ink); font-size: 12px; }
