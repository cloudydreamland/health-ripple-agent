    ...
    <svg viewBox="0 0 980 470" class="ridge-svg" preserveAspectRatio="xMidYMid meet">
      <defs>
        <pattern id="risk-hatch" width="7" height="7" patternTransform="rotate(45)" patternUnits="userSpaceOnUse">
          <rect width="7" height="7" fill="rgba(244,105,92,0.07)" />
          <line x1="0" y1="0" x2="0" y2="7" stroke="rgba(244,105,92,0.3)" stroke-width="1.4" />
        </pattern>
      ...
      <line :x1="xOf(RISK_LINE)" y1="112" :x2="xOf(RISK_LINE)" y2="442" stroke="var(--red)" stroke-width="1.2" stroke-dasharray="4 4" opacity="0.8" />
      <text :x="xOf(RISK_LINE) + 8" y="434" class="risk-zone-label">RISK ZONE ≥ 70</text>

      <!-- 纵向网格 -->
      <g v-for="t in axisTicks" :key="'grid-' + t">
        <line :x1="xOf(t)" y1="112" :x2="xOf(t)" y2="442" stroke="var(--ink)" stroke-opacity="0.06" stroke-width="1" />
      ...
      <g v-for="r in [...rings].reverse()" :key="'ridge-' + r.ring">
        ...
        <circle :cx="X0 - 5" :cy="baselineOf(r.ring)" r="2.4" fill="var(--muted)" />
      </g>
      ...
      <g v-if="topRisk">
        <line
          ...
          stroke="var(--red)" stroke-width="1.3" stroke-dasharray="3 4"
        />
        <g :transform="`translate(${Math.min(xOf(topRisk.intensity), X1 - 212)}, 132)`">
          <rect x="0" y="-14" width="200" height="22" rx="5" fill="#16283f" stroke="var(--red)" stroke-width="1.2" />
          <circle cx="10" cy="-3" r="3" fill="var(--red)" />
          <text x="18" y="1" class="top-chip">{{ topRisk.label }} · {{ topRisk.intensity.toFixed(1) }}</text>
        </g>
      </g>

      <!-- 基线与角标 -->
      <line :x1="X0" y1="442" :x2="X1" y2="442" stroke="var(--ink)" stroke-opacity="0.35" stroke-width="1.2" />
      <text :x="X1" y="104" text-anchor="end" class="axis axis-unit">NODE INTENSITY (RII 节点强度)</text>
      <g class="crosshair" opacity="0.5">
        <path d="M 128 112 h 8 M 132 108 v 8" stroke="var(--muted)" stroke-width="1" fill="none" />
        <path d="M 940 112 h 8 M 944 108 v 8" stroke="var(--muted)" stroke-width="1" fill="none" />
        <path d="M 128 442 h 8 M 132 438 v 8" stroke="var(--muted)" stroke-width="1" fill="none" />
        <path d="M 940 442 h 8 M 944 438 v 8" stroke="var(--muted)" stroke-width="1" fill="none" />
      </g>
    </svg>
  </div>
</template>
...
.ring-name { font-size: 11.5px; fill: #a9c0dc; font-weight: 600; font-family: var(--font-sans); }
.ring-count { font-size: 9px; fill: var(--faint); font-family: var(--font-mono); letter-spacing: 1px; }
.axis { font-size: 9.5px; fill: var(--faint); font-family: var(--font-mono); }
.axis-unit { letter-spacing: 1px; }
.risk-zone-label { font-size: 9.5px; fill: var(--red); font-family: var(--font-mono); letter-spacing: 1.4px; }
.top-chip { font-size: 10.5px; fill: var(--ink); font-family: var(--font-mono); }
