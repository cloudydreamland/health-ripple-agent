...
<template>
  <div class="ridge">
    <svg viewBox="0 0 980 470" class="ridge-svg" preserveAspectRatio="xMidYMid meet">
      ...
      <line :x1="xOf(RISK_LINE)" y1="112" :x2="xOf(RISK_LINE)" y2="442" stroke="#f4695c" stroke-width="1.2" stroke-dasharray="4 4" opacity="0.8" />
      <text :x="xOf(RISK_LINE) + 8" y="434" class="risk-zone-label">RISK ZONE ≥ 70</text>

      <!-- 纵向网格 -->
      <g v-for="t in axisTicks" :key="'grid-' + t">
        <line :x1="xOf(t)" y1="112" :x2="xOf(t)" y2="442" stroke="#dbe9f9" stroke-opacity="0.06" stroke-width="1" />
      ...
      <g v-for="r in [...rings].reverse()" :key="'ridge-' + r.ring">
        ...
        <circle :cx="X0 - 5" :cy="baselineOf(r.ring)" r="2.4" fill="#7e93ab" />
      </g>
      ...
      <g v-if="topRisk">
        <line
          ...
          stroke="#f4695c" stroke-width="1.3" stroke-dasharray="3 4"
        />
        <g :transform="`translate(${Math.min(xOf(topRisk.intensity), X1 - 212)}, 132)`">
          <rect x="0" y="-14" width="200" height="22" rx="5" fill="#16283f" stroke="#f4695c" stroke-width="1.2" />
          <circle cx="10" cy="-3" r="3" fill="#f4695c" />
          <text x="18" y="1" class="top-chip">{{ topRisk.label }} · {{ topRisk.intensity.toFixed(1) }}</text>
        </g>
      </g>

      <!-- 基线与角标 -->
      <line :x1="X0" y1="442" :x2="X1" y2="442" stroke="#dbe9f9" stroke-opacity="0.35" stroke-width="1.2" />
      <text :x="X1" y="104" text-anchor="end" class="axis axis-unit">NODE INTENSITY (RII 节点强度)</text>
      <g class="crosshair" opacity="0.5">
        <path d="M 128 112 h 8 M 132 108 v 8" stroke="#7e93ab" stroke-width="1" fill="none" />
        <path d="M 940 112 h 8 M 944 108 v 8" stroke="#7e93ab" stroke-width="1" fill="none" />
        <path d="M 128 442 h 8 M 132 438 v 8" stroke="#7e93ab" stroke-width="1" fill="none" />
        <path d="M 940 442 h 8 M 944 438 v 8" stroke="#7e93ab" stroke-width="1" fill="none" />
      </g>
    </svg>
  </div>
</template>
...
.ring-count { font-size: 9px; fill: #5a708c; font-family: var(--font-mono); letter-spacing: 1px; }
.axis { font-size: 9.5px; fill: #5a708c; font-family: var(--font-mono); }
.axis-unit { letter-spacing: 1px; }
.risk-zone-label { font-size: 9.5px; fill: #f4695c; font-family: var(--font-mono); letter-spacing: 1.4px; }
.top-chip { font-size: 10.5px; fill: #dbe9f9; font-family: var(--font-mono); }
