    ...
    <template v-if="forecast && forecast.buckets.length">
      <svg :viewBox="`0 0 ${W} ${H}`" class="fc-svg">
        ...
        <g v-for="t in dayTicks" :key="t.label">
          <line :x1="t.x" :x2="t.x" :y1="PAD_T" :y2="H - PAD_B" stroke="var(--line-strong)" stroke-width="1" stroke-dasharray="2 5" />
          <text :x="t.x" :y="H - PAD_B + 15" text-anchor="middle" class="fc-tick">{{ t.label }}</text>
        </g>
        <!-- 强度基线 -->
        <line :x1="PAD_L" :x2="W - 12" :y1="H - PAD_B" :y2="H - PAD_B" stroke="var(--muted)" stroke-width="1.2" />
        ...
        <path :d="linePath" fill="none" stroke="var(--red)" stroke-width="2" />
        <!-- 依从性沙盘曲线：守护被执行后的确定性重算（虚线） -->
        <path v-if="sandboxLinePath" :d="sandboxLinePath" fill="none" stroke="var(--cyan)" stroke-width="2" stroke-dasharray="5 4" />
        <circle v-if="peakMark" :cx="points[forecast.peak.hourOffset].x" :cy="points[forecast.peak.hourOffset].y" r="4" fill="var(--red)" stroke="var(--card-inset)" stroke-width="1.5" />
        <g v-if="peakMark">
          <line :x1="peakMark.x" :x2="peakMark.x" :y1="peakMark.y - 4" :y2="peakMark.y + 4" stroke="var(--red)" stroke-width="1" />
...
.fc-svg { width: 100%; }
.fc-tick { font-size: 10px; fill: var(--muted); font-family: var(--font-mono); }
...
.sb-slider { flex: 1; min-width: 160px; accent-color: var(--cyan); height: 4px; cursor: pointer; }
