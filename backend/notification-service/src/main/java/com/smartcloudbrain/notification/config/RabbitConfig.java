...
public class RabbitConfig {
  ...
  @Bean
  Queue rippleDispatchQueue() {
    return new Queue(RabbitTopology.RIPPLE_DISPATCH_QUEUE, true, false, false, Map.of(
        "x-dead-letter-exchange", RabbitTopology.DEAD_EXCHANGE
    ));
  }
  ...
  @Bean
  Binding rippleDispatchBinding(
      @Qualifier("rippleDispatchQueue") Queue rippleDispatchQueue,
      @Qualifier("domainExchange") TopicExchange domainExchange
  ) {
    return BindingBuilder.bind(rippleDispatchQueue)
        .to(domainExchange)
        .with(RabbitTopology.ROUTING_RIPPLE_DISPATCH);
  }
