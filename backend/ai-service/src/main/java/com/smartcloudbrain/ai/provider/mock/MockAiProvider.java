 ...
 *
 * 优先级0：急症红色指征（神经/消化道/哮喘持续状态/颅压危象）→ EMERGENCY 分流
 * 优先级1：儿科（患儿主诉优先按年龄层路由，避免成人规则误套）
 * 优先级2：妊娠相关 → 产科
 * 优先级3：心血管危险症状 → 心内科（胸闷需危重伴随症才升急，稳定型属常规评估）
 * 优先级4：代谢症状 → 全科初诊
 * 优先级5：呼吸症状 → 呼吸内科
 * 优先级6：消化症状 → 消化内科方向（全科首诊转诊）
...
public class MockAiProvider implements AiProvider {
  ...
  private static final List<String> CARDIO_DANGER = List.of(
      "胸痛", "胸闷", "心悸", "气短", "chest pain", "dyspnea", "palpitation");
  ...
  public TriageResponse triage(TriageRequest request, PromptResolveResponse prompt) {
    ...
    // 优先级3：心血管——胸痛/晕厥单独即急症；胸闷/心悸需危重伴随症才升急（稳定型属常规评估）
